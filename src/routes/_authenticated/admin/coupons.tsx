  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={add} className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-5 md:grid-cols-3">
        ...
        <label className="text-xs text-cream/60">Starts at<input type="datetime-local" className="input-base mt-1" value={draft.starts_at} onChange={(e) => setDraft({ ...draft, starts_at: e.target.value })} /></label>
        <label className="text-xs text-cream/60">Expires at<input type="datetime-local" className="input-base mt-1" value={draft.expires_at} onChange={(e) => setDraft({ ...draft, expires_at: e.target.value })} /></label>
