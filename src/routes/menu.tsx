        ...
        <div className="container-page">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            {count > 0 && (
              <button
                onClick={open}
                className="inline-flex items-center justify-center gap-2 rounded-full gradient-gold px-5 py-3 text-sm font-semibold text-primary-foreground"
              >
          ...
          data.categories.map((c) => {
            ...
            return (
              <div key={c.id} className="mb-12">
                <h2 className="font-display text-2xl md:text-3xl text-gold">{c.name}</h2>
...
function FoodCard({ food, onAdd }: { food: Food; onAdd: ReturnType<typeof useCart>["add"] }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition hover:border-gold/40 hover:shadow-lg hover:shadow-black/20">
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        ...
        {food.is_featured && (
          <span className="absolute left-3 top-3 rounded-full bg-black/70 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-gold backdrop-blur">
            Chef's pick
          </span>
        )}
        {food.discount_price && (
          <span className="absolute right-3 top-3 rounded-full bg-destructive/90 px-2.5 py-1 text-[10px] font-bold uppercase text-white">
            Save {formatPKR(food.price - food.discount_price)}
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-lg leading-tight">{food.name}</h3>
          {food.spice && food.spice !== "none" && (
            <span className="inline-flex items-center gap-0.5 text-orange-400" title={`Spice: ${food.spice}`}>
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          <div className="flex items-baseline gap-2">
            <span className="font-display text-xl text-gold">{formatPKR(price)}</span>
            {food.discount_price && (
              <span className="text-xs text-muted-foreground line-through">{formatPKR(food.price)}</span>
            )}
          </div>
          <button
            onClick={() => onAdd({ id: food.id, name: food.name, price, image_url: food.image_url })}
            className="inline-flex items-center gap-1.5 rounded-full gradient-gold px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow hover:brightness-110"
          >
            <Plus size={14} /> Add
          </button>
        </div>
